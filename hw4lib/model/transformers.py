...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dec_layers     = torch.nn.ModuleList([SelfAttentionDecoderLayer(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)])
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        return self.final_linear(seq_out), runnint_att
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layers = torch.nn.ModuleList([SelfAttentionEncoderLayer(d_model, num_encoder_heads, d_ff_encoder, dropout) for _ in range(num_encoder_layers)])
        self.dec_layers = torch.nn.ModuleList([CrossAttentionDecoderLayer(d_model, num_decoder_heads, d_ff_decoder, dropout) for _ in range(num_decoder_layers)])
