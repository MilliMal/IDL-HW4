...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            feat = np.load(os.path.join(self.fbank_dir, self.fbank_files[i])) # NotImplementedError
            ...
            if self.partition != "test-clean":
                ...
                shifted = [self.sos_token] + tokenized # NotImplementedError
                golden  = tokenized + [self.eos_token] # NotImplementedError    
                # Store the shifted and golden transcripts
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [feat.transpose(0, 1) for feat, _, _ in batch] # B x T x F
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.LongTensor([t.shape[0] for t in batch_shifted]) # B  

            # TODO: Pad transcripts to create a batch of fixed-length padded transcripts
            padded_shifted     = pad_sequence(batch_shifted, batch_first=True, padding_value=0) # B x T
            padded_golden      = pad_sequence(batch_golden, batch_first=True, padding_value=0) # B x T
