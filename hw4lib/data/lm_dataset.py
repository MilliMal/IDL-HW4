...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        for file in tqdm(self.text_files):
            ...
            full_path = os.path.join(self.text_dir, file)
            transcript = str(np.load(full_path))  # NotImplementedError
            ...
            tokenized = self.tokenizer.encode(transcript) # NotImplementedError
        ...
        self.length = len(self.transcripts_shifted) # NotImplementedError
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        shifted_transcripts, golden_transcripts = zip(*batch) # NotImplementedError
